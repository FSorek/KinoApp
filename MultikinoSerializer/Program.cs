...
namespace MultikinoSerializer
{
    // To parse this JSON data, add NuGet 'Newtonsoft.Json' then do:
    //
    //    using QuickType;
    //
    //    var data = Multikino.FromJson(jsonString);

        public class Program
        {
            static void Main(string[] args)
            {
                var httpWebRequest = (HttpWebRequest)WebRequest.Create("https://multikino.pl/data/filmswithshowings/14");
                ...
                try
                {
                    ...
                    var data = Multikino.FromJson(respStr);
                    Console.WriteLine(data.Films[3].Title);
                }
                catch (Exception e)
                {
